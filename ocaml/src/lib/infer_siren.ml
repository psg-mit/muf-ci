...
let pp_mdistr_float : float mdistr -> string expr =
fun d ->
  let rec get_string : float mdistr -> string =
  fun d -> 
    let pp_distribution : float distribution -> string =
    fun d ->
      match d with
      | Normal (ExConst(mu), ExConst(var)) ->
        Format.sprintf "Gaussian(%f, %f)" mu var
      ...
      | Delta (ExConst(a)) ->
        (* TODO: any way to print the value? *)
        Format.sprintf "Delta (%f)" a 
      | Mixture (l) -> 
        Format.sprintf "Mixture([%s])" 
          (String.concat "; " (List.map (fun (e, p) -> 
            Format.sprintf "(%s, %f)" (get_string e) p) l))
      | Sampler _ | MvNormal _ -> failwith "not implemented"
      | _ -> failwith "pp_mdistr_float: not marginal"
    in
    match d with
    | ExConst a -> Format.sprintf "Const (%f)" a
    | ExRand rv -> Format.sprintf "%s" (pp_distribution rv.distr)
    | ExAdd (e1, e2) -> Format.sprintf "Add(%s, %s)" (get_string e1) (get_string e2)
    | ExMul (e1, e2) -> Format.sprintf "Mul(%s, %s)" (get_string e1) (get_string e2)
    | ExDiv (e1, e2) -> Format.sprintf "Div(%s, %s)" (get_string e1) (get_string e2)
  ...
  fun e ->
    ...
    fun e ->
      ...
      | ExRand rv -> 
        ...
        | Mixture l -> 
          (* Print list *)
          (* Format.printf "Mixture([%s])\n" 
            (String.concat "; " (List.map (fun (e, p) -> 
              Format.sprintf "(%s, %f, %f)" (get_const (pp_mdistr_float e)) (mean_float' e) p) l)); *)
          List.fold_left (fun acc (d, p) -> acc +. (mean_float' d) *. p) 0. l
