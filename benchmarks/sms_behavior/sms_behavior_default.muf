...
val mse = fun ((true_lambda1, true_lambda2, true_tau), d) ->
  ...
  let total_mse = div_float(add_float(add_float(lambda1_mse, lambda2_mse), tau_mse), 3.) in
...
val sms_behavior = stream {
  ...
  step ((first, lambda1, lambda2, tau), (i, count)) = 
    let (lambda1, lambda2) = 
      if first then
        (sample("lambda1", exponential(alpha(()))), sample("lambda2", exponential(alpha(()))))
      else
        (lambda1, lambda2)
    in
    let tau = if first then sample("tau", uniform_int(0, n_data(()))) else tau in
