...
val make_observations = fun (acc, yobs) ->
  ...
  let exact switch <- bernoulli(0.3) in
...
let exact mu1 <- gaussian(2.75, 0.5) in
let exact mu2 <- gaussian(-2.75, 0.5) in

(* Improper prior *)
let approx var1 <- gamma(1., 1.) in
let exact var2 <- gamma(1., 1.) in
