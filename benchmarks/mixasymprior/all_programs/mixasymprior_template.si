...
val make_observations = fun (acc, yobs) ->
  ...
  let PLACEHOLDER switch <- bernoulli(0.3) in
...
let PLACEHOLDER mu1 <- gaussian(2.75, 0.5) in
let PLACEHOLDER mu2 <- gaussian(-2.75, 0.5) in

(* Improper prior *)
let PLACEHOLDER var1 <- gamma(1., 1.) in
let PLACEHOLDER var2 <- gamma(1., 1.) in
