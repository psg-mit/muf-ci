...
val debug = fun () -> false
val true_outlier_prob = fun () -> 0.0825916853557
...
val outlier = stream {
  ...
  step ((first, xt, outlier_prob), yobs) =
    let xt = approx(sample ("xt", gaussian (if first then (const (0.), const(2500.)) else (xt, const(1.))))) in
    let outlier_prob = if first then approx(sample ("outlier_prob", beta (100., 1000.))) else outlier_prob in
