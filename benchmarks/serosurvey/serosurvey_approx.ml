    ...
    Muflib.step =
      (fun
         ...
         let (sens, fpr) =
           if first
           then
             ((approx (Muflib.prob_op sample prob ("sens", (beta (1., 1.))))),
               (approx (Muflib.prob_op sample prob ("fpr", (beta (1., 1.))))))
           else (sens, fpr) in
         ...
         let p = sigmoid p' in
         let true_pos =
           approx (Muflib.prob_op sample prob ("true_pos", (bernoulli p))) in
         let () =
